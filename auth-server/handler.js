/**
 * To push changes from this file:
 * In Console navigate to meet/auth-server
 * type "serverless deploy"
 * 
 * To see info on endpoints:
 * In Console navigate to meet/auth-server
 * type "serverless info"
 */
...
const credentials = {
  ...
  redirect_uris: ["https://lukehthompson.github.io/meet/"],
  javascript_origins: ["https://lukehthompson.github.io", "http://localhost:3000"],
};
...
module.exports.getAccessToken = async (event) => {
  // The values used to instantiate the OAuth2Client are at the top of the file
  const oAuth2Client = new google.auth.OAuth2(
    client_id,
    client_secret,
    redirect_uris[0]
...
module.exports.getCalendarEvents = async (event) => {
  ...
  // Wish I understood better what is happening here in line 130 as I don't see any "token" as a piece of the URI anywhere...
  const access_token = decodeURIComponent(`${event.pathParameters.access_token}`);
  oAuth2Client.setCredentials({ access_token });

  return new Promise((resolve, reject) => {
    /**
     * Exchange access_token for calendar event(s) with a "callback" after the exchange,
     * The callback in this case is an arrow function with the results as parameters: "err" and "event".
     */

    oAuth2Client.getToken(access_token, () => {
